...
export const GATEWAYS: readonly GatewayRow[] = [
  {
    ...
    cells: {
      ...
      alerts: { status: "partial", note: "The budget-limits docs now ship a worked Prometheus/Alertmanager rule (BudgetNearlyExhausted) on the budget metric, so a cap-hit can be alerted — but only by wiring Alertmanager yourself; no built-in Slack/webhook/email notification ships in the proxy. Same posture as Kong: metrics + a documented self-wired path, not first-party notification." },
      ...
      "vertex-invoice": { status: "yes", note: "Operator-set Vertex billing labels ship in v1.5.0 (2026-08-27): finalTransformations (#2912, merged 2026-08-10) sets labels on the generateContent request after provider conversion, and they reach the Google Cloud billing export; documented in website PR #931. The replace form with operator-resolved values ({tenant: jwt.sub, cost_center: 'platform'}) cannot be forged by the caller, which meets the bar. Caveat kept in the open: unlike an STS session tag, a final transformation fails open — an expression that errors drops the field and the request still reaches Vertex, so a mistyped field silently drops attribution, and the merge form forwards caller labels the operator did not claim. Scored yes on the replace form's operator guarantee; the fail-open posture is the reason it is not stronger." },
  ...
  },
  {
    ...
    cells: {
      ...
      "vertex-invoice": { status: "partial", note: "LiteLLM forwards a labels field (and converts string metadata to labels) into Vertex generateContent for GCP billing, but docs show no server-side operator label pinning, so a caller can influence the labels." },
  ...
  },
  {
    ...
    lastVerified: "2026-08-28",
    cells: {
      "enforced-keys": { status: "partial", note: "OIDC/key-auth can require a consumer and reject anonymous, and auth plugins can be scoped globally (gateway-wide), not only per-route; partial because full OIDC is Kong Enterprise and the enforcement is opt-in plugin config rather than a baseline default." },
  ...
  },
  {
    ...
    lastVerified: "2026-08-28",
    cells: {
      "enforced-keys": { status: "partial", note: "Standard spend limits read caller-supplied cf-aig-metadata (trusted, attribution optional); enforced verified identity exists only via Cloudflare Access (identity-aware controls, open beta since 2026-08-05), which is gated to an Access-protected custom domain rather than the general endpoint." },
      "jwt-values": { status: "partial", note: "Identity-driven per-user spend limits derive attribution from a Cloudflare Access verified login (server-verified cf.user_id, IdP groups), not caller metadata; open beta as of 2026-08-05, still opt-in and custom-domain-gated." },
      "static-values": { status: "partial", note: "cf-aig-metadata is caller-supplied and trusted so pinned values can be overridden on the standard path, though Cloudflare Access (open beta) binds verified identity server-side that the caller cannot forge." },
  ...
  },
  {
    ...
    lastVerified: "2026-08-28",
    cells: {
      "enforced-keys": { status: "yes", note: "enforce_auth_on_inference (the canonical control; enforce_governance_header is deprecated) leaves no anonymous path to a model, and operator-configured required headers reject a missing one pre-provider with a structured 400 {\"type\":\"missing_required_headers\"}; opt-in, not on by default." },
      "jwt-values": { status: "partial", note: "OIDC via Okta/Entra/Auth0/Keycloak/Google with configurable JWT claim fields (userIdField/rolesField/teamIdsField), attribute-to-role/team/BU mappings, and inbound SCIM 2.0 attribute mapping; claim-to-attribution mapping is now fully documented, so partial reflects only the enterprise gating." },
      ...
      alerts: { status: "yes", note: "Built-in CEL-based governance alert rules send budget/failure notifications to Slack, Microsoft Teams, PagerDuty, and generic webhooks, not just self-wired metrics." },
      ...
      "live-changes": { status: "partial", note: "Config applies at runtime with no restart (add provider / revoke key take effect on next request) plus memberlist-gossip cluster mode and zero-downtime rolling deploys, but governance counters replicate with eventual consistency (nodes converge within seconds), so cross-node budget enforcement can briefly lag." },
    },
  },
  {
    id: "helicone",
    ...
    cells: {
      ...
      alerts: { status: "yes", note: "Built-in cost/error/latency/token threshold alerts route to Slack and email, but alerting is gated to the Pro plan ($79/mo) and above." },
