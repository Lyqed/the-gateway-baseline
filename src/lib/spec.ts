...
export const SPEC_CHECKS: readonly SpecCheck[] = [
  ...
  },
  {
    ...
    "rationale": [
      "A spend figure computed from a price table is an estimate. The number AWS charges is in the Cost and Usage Report, and it is the only number that ends a billing dispute. A gateway can meter tokens and multiply by prices, and finance can build chargeback on that arithmetic, but the first discrepancy turns the whole ledger into a negotiation. When the attribution value rides to the bill itself, the cost per spender is a fact before anyone asks whether the spend was worth it.",
