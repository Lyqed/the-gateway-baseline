...
export const PASSES: readonly VerificationPass[] = [
  {
    date: "2026-08-28",
    note: "Full board re-verification: every cell across all eight gateways (72 cells) re-checked against live docs, GitHub releases, and repo activity. No status changed: every yes/partial/no held. agentgateway GB-6 (alerted) moved no → partial — the budget-limits docs now ship a worked Prometheus/Alertmanager rule (BudgetNearlyExhausted), so a cap-hit is alertable, but only by wiring Alertmanager yourself; no first-party Slack/webhook/email notification ships (same posture as Kong). Note-text corrected where the wording had gone stale though the verdict held: Bifrost GB-1 (the flag is now enforce_auth_on_inference, not the deprecated enforce_governance_header), Bifrost GB-2 (claim mapping is now fully documented, so partial reflects only enterprise gating), Bifrost GB-6 and Helicone GB-6 (channel lists corrected to what the docs actually list), Kong GB-1 (auth plugins can be scoped globally, so partial rests on enterprise-gating not per-route scope), and Cloudflare GB-1/2/3 (Access verified-identity advanced closed beta → open beta, shipped 2026-08-05, still custom-domain-gated). Several truncated note sentences were completed. Cloudflare GB-6 was specifically checked and still has no spend-breach notification; LiteLLM GB-7 still no (PRs #32797 and #34073 both still open); Helicone confirmed dormant (both repos last pushed 2025).",
  },
  {
    date: "2026-08-27",
